...
async def receive_app_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document:
        ...
        try:
            ...
            # Скачиваем напрямую с Telegram API (без Cloudflare)
            file_url = new_file.file_path
            direct_download_url = f"https://api.telegram.org/file/bot{BOT_TOKEN}/{file_url}"
            resp = requests.get(direct_download_url, timeout=DOWNLOAD_TIMEOUT)
